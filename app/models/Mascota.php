...
class Mascota {
     ...
     * NOTA: usa la columna 'dueño_id' tal y como está en tu esquema.
     */
    public function getTodasConDueño(): array {
        $sql = "SELECT m.*, u.nombre AS nombre_dueno, u.apellido AS apellido_dueno, u.telefono AS telefono_dueno
                FROM mascotas m
                LEFT JOIN usuarios u ON m.dueño_id = u.id
    ...
    public function getByDueno($dueno_id): array {
        $sql = "SELECT * FROM mascotas WHERE dueño_id = ?";
    ...
    public function crear($dueno_id, $data) {
        $sql = "INSERT INTO mascotas (dueño_id, nombre, especie, raza, edad, peso, foto, notas, creado_en)
                VALUES (?, ?, ?, ?, ?, ?, NULL, ?, NOW())";
        $stmt = $this->db->prepare($sql);
        $stmt->execute([
            $dueno_id,
    ...
    /**
     * Mascotas asignadas a veterinario (según citas futuras).
    ...
    public function getAssignedToVeterinario($veterinarioId, $limite = 8): array {
        $sql = "
            ...
            INNER JOIN usuarios u ON m.dueño_id = u.id
        ...
        try {
            ...
            // fallback concatenando el límite (seguro porque casteamos a int)
    ...
    /**
     * Búsqueda segura por nombre/especie/raza o nombre del dueño.
