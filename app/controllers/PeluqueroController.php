...
class PeluqueroController extends Controller
{
    ...
    {
        // Usa categoria si existe en el esquema actual; fallback por nombre
        return "( (CASE WHEN (
                      SELECT COUNT(*) FROM information_schema.COLUMNS 
                      WHERE TABLE_SCHEMA = DATABASE() AND TABLE_NAME='servicios' AND COLUMN_NAME='categoria'
                   ) > 0 THEN COALESCE(s.categoria,'') ELSE '' END) = 'peluqueria'
                  OR LOWER(s.nombre) LIKE '%peluquer%'
                  OR LOWER(s.nombre) LIKE '%bañ%'
                  OR LOWER(s.nombre) LIKE '%ban%'
                  OR LOWER(s.nombre) LIKE '%cort%'
                  OR LOWER(s.nombre) LIKE '%spa%')";
    }
